...
export const fetchActividades = async (token) => {
  try {
    const res = await api.get("/estudiante/actividades");
...
export const fetchActividadesPorClase = async (claseId, token) => {
  try {
    const res = await api.get(`/estudiante/actividades/clase/${claseId}`);
    return res.data.actividades || [];
  } catch (error) {
...
export const enviarRespuestaActividad = async (
  ...
  try {
    const res = await api.post(
      `/estudiante/actividades/${actividadId}/responder`,
      {
        respuesta,
      }
    );
